...
Problema 1.2, Laborator 8 - client
Data ultimei modificari: 12.05.2021
Stadiu: Rezolvata - nu stiu daca am satisfacut cerinta ???
...
void executareArgumente(char bufArg[MAXLINE][MAXLINE], int cnt)
{
    ...
    args[0] = "./server";///nume executabil server
    for (int i = 0; i < cnt; i++)
    {
    ...
    execlp(args[0], "server", args[1], args[2], args[3]);
}
int main(void)
{
